...
public protocol KVFetcher_Protocol: class, KeyValueProtocol {
	...
	/// INTERNAL: A list of closures that will be executed. Internal, please don't use this.
...
extension KVFetcher_Protocol {
	...
	func _executeTimeoutFetchValue(for key: Key, completion: ValueCompletion!) {
		...
		_executeFetchValue(for: key, completion: { tryCompleting($0) })
		delay(timeout) { tryCompleting(nil) }
	}
	...
	public func executeNextQueuedClosure() {
	...
	/// KVFetcher_Protocol: fetches multiple values for multiple keys
	...
	/// KVFetcher_Protocol: blocks the main thread and UI until the value is fetched.
	@discardableResult
