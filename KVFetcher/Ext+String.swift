...
extension String {
    ...
    func nilIfEquals(_ match: String) -> String? { return self == match ? nil : self }
    func nilIfContains(_ match: String) -> String? { return self.contains(match) ? nil : self }
...
extension String.Encoding {
    static var all: [String.Encoding] {
        return [.ascii,.nextstep,.japaneseEUC,.utf8,.isoLatin1,.symbol,.nonLossyASCII,.shiftJIS,.isoLatin2,.unicode,.windowsCP1251,.windowsCP1252,.windowsCP1253,.windowsCP1254,.windowsCP1250,.iso2022JP,.macOSRoman,.utf16,.utf16BigEndian,.utf16LittleEndian,.utf32,.utf32BigEndian,.utf32LittleEndian,]
    }
    
    static func fromString(_ str: String) -> String.Encoding {
        let str = str.removing("-", "windows", ".", "/").lowercased()
        let allCases = String.Encoding.all
        let tuples = allCases.map {($0, "\($0)".removing("-", ".", "/", "windows").lowercased()) }
        if let encoding = tuples.first(where: { $0.1 == str})?.0 {
            print("Encoding search: found \(encoding)")
            return encoding
        }
        //let sorted = tuples.sorted { $0.1.distance(between: str) > $1.1.distance(between: str) }
        //let bestMatch = sorted.first!.0
        //let bestMatch = String.Encoding.utf8
        fatalError()
        //print("Encoding Best match: found \(bestMatch)")
        //return bestMatch
    }
}
